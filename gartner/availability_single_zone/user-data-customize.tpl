...
for a in {randrw,}
 do
  ...
  for bs in {${BS}}
   do
    for iodpth in {${IODPTH}}
     do
     for njobs in {${NJOBS}}
      do
      printf "=============="
      #modify output file names accordingly
      fio --filename=/dev/vdd --direct=1 --rw=$a --rwmixread=70 --rwmixwrite=30 --bs=$bs --iodepth=$iodpth --numjobs=$njobs --runtime=${RUNTIME} --time_based --group_reporting --name=throughput-test-job --eta-newline=1 --ioengine=posixaio --output-format=json,normal > ~/outputs/$a-1TB_custumIOPs_vol_$bs-bs_$iodpth-iodepth_$njobs-numjobs.json
 ...
 filename=fio-${zone}-$a-1TB_custumIOPs_vol_$(date +'%d-%m-%y:%R').tar.gz
