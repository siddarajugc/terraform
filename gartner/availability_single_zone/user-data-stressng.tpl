...
# run the data test
stress-ng --cpu 4 --vm 2 --hdd 1 --fork 8 --timeout 10m --metrics --yaml /tmp/stressng_result
return_value=$?
...
stressng_result=$(date +'%d-%m-%y:%R')-stressng_result
$s3cmd_cmd put /tmp/stressng_result s3://$bucket_name/$region/$service/$sku/$year/$month/$day/$test_data/$stressng_result
return_value=$?
if [ "$return_value" -ne 0 ]; then
    sleep 10
    $s3cmd_cmd put /tmp/stressng_result s3://$bucket_name/$region/$service/$sku/$year/$month/$day/$test_data/$stressng_result
