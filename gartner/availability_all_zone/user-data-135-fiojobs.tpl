...
for a in {randrw,}
 do
  ...
  for bs in {4K,8K,16K,32K,64K,128K,256K,512K,1M}
   do
    for iodpth in {1,4,8,16,64}
     do
     for njobs in {1,4,8}
      do
      printf "=============="
      #modify output file names accordingly
      fio --filename=/dev/vdd --direct=1 --rw=$a --rwmixread=70 --rwmixwrite=30 --bs=$bs --iodepth=$iodpth --numjobs=$njobs --runtime=300 --time_based --group_reporting --name=throughput-test-job --eta-newline=1 --ioengine=posixaio --output-format=json > ~/outputs/$a-8TB_custumIOPs_vol_$bs-bs_$iodpth-iodepth_$njobs-numjobs.json
 ...
 filename=fio-$a-8TB_custumIOPs_vol_$(date +'%d-%m-%y:%R').tar.gz
