#!/bin/bash
if [ "$#" -lt 1 ]; then
  echo "Usage:"
  echo "$0 dal|wdc|..|syd tfvars_dir s3cmd_dir" >&2
  echo
  echo e.g. ./terraform.sh wdc . /var/lib/jenkins/s3cmd
  echo
  exit 1
fi
export IBMCLOUD_RESOURCE_MANAGEMENT_API_ENDPOINT=https://resource-controller.cloud.ibm.com
export IBMCLOUD_IAM_API_ENDPOINT=https://iam.test.cloud.ibm.com
export IBMCLOUD_RESOURCE_CONTROLLER_API_ENDPOINT=https://resource-controller.cloud.ibm.com
export IBMCLOUD_GT_API_ENDPOINT=https://tags.global-search-tagging.cloud.ibm.com
export IBMCLOUD_RESOURCE_CATALOG_API_ENDPOINT=https://globalcatalog.cloud.ibm.com
export IBMCLOUD_IAM_API_ENDPOINT=https://iam.cloud.ibm.com
export IBMCLOUD_IS_NG_API_ENDPOINT="https://eu-gb.iaas.cloud.ibm.com/v1"


# get start date
year=$(date -u +%Y)
month=$(date -u +%m)
day=$(date -u +%d)
starttime=$(date -u +%s)

uuid=$starttime

cd /availability_perf_prefil
ls | grep logs
return_value=$?
if [ "$return_value" -ne 0 ]; then
    mkdir logs
fi

region_prefix=$1
tfvars_dir=$WORKDIR
s3cmd_dir=$WORKDIR

tempfile_tfvars="logs/${region_prefix}-$starttime.tfvars"
TF_LOG_APPLY="logs/$uuid-apply.logs"
TF_OUTPUT_FILE="logs/output.json"
TF_LOG_DESTROY="logs/$uuid-destroy.logs"
#TF_VSI_ID="logs/${uuid}/vsi_id.logs"

cp -v ${tfvars_dir}/terraform_${region_prefix}.tfvars $tempfile_tfvars
chmod 600 $tempfile_tfvars


while IFS= read -r line; do export $(echo $line | tr -d '"'); done < "$tempfile_tfvars"
terraform workspace new $region_prefix-$starttime

terraform init

terraform apply -auto-approve -no-color -var-file=$tempfile_tfvars 2>>$TF_LOG_APPLY 1>>$TF_LOG_APPLY
terraform output -json > $TF_OUTPUT_FILE
#cat $TF_OUTPUT_FILE | jq '.VSI_ID.value[].id' > $TF_VSI_ID
echo "Terraform tfvars file: $tempfile_tfvars"
echo "Terraform log file: $TF_LOG_APPLY"
echo "terraform apply -auto-approve -no-color -var-file=$tempfile_tfvars"
echo "terraform destroy -auto-approve -no-color -var-file=$tempfile_tfvars"
sleep $SLEEP_TIME
#exit 0
return_value=$?

terraform output
echo "terraform destroy -no-color -auto-approve -var-file=$tempfile_tfvars"

#exit 0
if [ "$return_value" -ne 0 ]; then
    mv $TF_LOG_PATH logs
fi
echo "Created resources"

export TF_LOG_PATH="tf_destroy_trace.log"
#terraform destroy -no-color -auto-approve -var-file=$tempfile_tfvars 2>>$TF_LOG_DESTROY 1>>$TF_LOG_DESTROY
#return_value=$?

finishtime=$(date -u +%s)
let elapsedtime=$finishtime-$starttime

if [ "$return_value" -ne 0 ]; then
    mv $TF_LOG_PATH logs
fi
echo "destroyed resources"

# delete the temp files
rm -f $tempfile_tfvars